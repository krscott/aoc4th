...
: crates-exec-move ( 'line len 'self -- )
  ...
  2drop                             ( 'self amt src dest )

  rot 0 do                          ( 'self src dest )
    over 3 pick                     ( 'self src dest src 'self )
    fixlist-get                     ( 'self src dest 'list )
    fixlist-pop                     ( 'self src dest c )
    over 4 pick                     ( 'self src dest c dest )
    fixlist-get                     ( 'self src dest c 'list )
    fixlist-push                    ( 'self src dest )
  loop
